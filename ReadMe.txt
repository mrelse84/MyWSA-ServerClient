================================================================================
    MFC 라이브러리 : MyWSA-ServerClient 프로젝트 개요
================================================================================

응용 프로그램 마법사에서 이 MyWSA-ServerClient 응용 프로그램을 
만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 
프로그램작성을 위한 기본 구조를 제공합니다.

이 파일에는 MyWSA-ServerClient 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

MyWSA-ServerClient.vcxproj
...
MyWSA-ServerClient.vcxproj.filters
...
MyWSA-ServerClient.h
    응용 프로그램의 기본 헤더 파일입니다. 여기에는 다른 프로젝트 관련
    헤더(Resource.h 포함)가 들어 있고 CMyWSAServerClientApp 응용 프로그램
    클래스를 선언합니다.

MyWSA-ServerClient.cpp
    응용 프로그램 클래스 CMyWSAServerClientApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.

MyWSA-ServerClient.rc
...
res\MyWSA-ServerClient.ico
    응용 프로그램의 아이콘으로 사용되는 아이콘 파일입니다. 이 아이콘은
    주 리소스 파일인 MyWSA-ServerClient.rc에 의해 포함됩니다.

res\MyWSAServerClient.rc2
...
MyWSA-ServerClientDlg.h, MyWSA-ServerClientDlg.cpp - 대화 상자
    이 파일에는 CMyWSAServerClientDlg 클래스가 들어 있습니다. 이 클래스는
    응용 프로그램의 주 대화 상자 동작을 정의합니다. 대화 상자의 템플릿은
    Microsoft Visual C++에서 편집할 수 있는 MyWSA-ServerClient.rc에 있습니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 MyWSA-ServerClient.pch와
